...
class MIP_Model(object):
    ...
    def _add_variables(self):
        ...
        '''
        self.num_cuts = self.model.integer_var(lb=0, ub=self.max_cuts+0.1, name='num_cuts')

        self.subcircuit_counter = {}
        for subcircuit in range(self.num_subcircuit):
            self.subcircuit_counter[subcircuit] = {}

            self.subcircuit_counter[subcircuit]['original_input'] = self.model.integer_var(lb=0, ub=self.max_subcircuit_width, name='original_input_%d'%subcircuit)
            self.subcircuit_counter[subcircuit]['rho'] = self.model.integer_var(lb=0, ub=self.max_subcircuit_width, name='rho_%d'%subcircuit)
            self.subcircuit_counter[subcircuit]['O'] = self.model.integer_var(lb=0, ub=self.max_subcircuit_width, name='O_%d'%subcircuit)
            self.subcircuit_counter[subcircuit]['d'] = self.model.integer_var(lb=0.1, ub=self.max_subcircuit_width, name='d_%d'%subcircuit)
            ...
            if subcircuit>0:
                ...
                self.subcircuit_counter[subcircuit]['build_cost_exponent'] = self.model.integer_var(lb=lb, ub=ub, name='build_cost_exponent_%d'%subcircuit)
    ...
    def _add_constraints(self):
        ...
        for v in range(self.n_vertices):
            ...
            self.model.add_constraint(self.model.sum(self.vertex_var[i][v] for i in range(self.num_subcircuit)) == 1, ctname=ctName)
        ...
        '''
        self.model.add_constraint(self.num_cuts == 
        self.model.sum(
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            '''
            self.model.add_constraint(self.subcircuit_counter[subcircuit]['original_input'] - 
            ...
            self.subcircuit_counter[subcircuit]['original_input'] - self.subcircuit_counter[subcircuit]['rho']==0,
            ctname='cons_subcircuit_d_qubits_%d'%subcircuit)

            if self.max_subcircuit_cuts is not None:
                self.model.add_constraint(self.subcircuit_counter[subcircuit]['num_cuts'] -
                self.subcircuit_counter[subcircuit]['rho'] - self.subcircuit_counter[subcircuit]['O']==0,
                ctname='cons_subcircuit_num_cuts_%d'%subcircuit)

            if self.max_subcircuit_size is not None:
                self.model.add_constraint(self.subcircuit_counter[subcircuit]['size'] -
                self.model.sum([self.vertex_var[subcircuit][v] for v in range(self.n_vertices)]) == 0,
