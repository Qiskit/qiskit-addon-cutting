...
To perform circuit cutting, one must place cuts in the graph representing a quantum circuit.  The circuit is then transformed into a collection of circuits which are executed on hardware.  The results of the original circuit are reconstructed during post-processing, resulting in the desired quantity (e.g., the expectation value of a joint observable).
