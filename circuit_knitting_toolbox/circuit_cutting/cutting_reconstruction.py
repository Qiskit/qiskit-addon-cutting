...
def reconstruct_expectation_values(
    ...
    Args:
        :parameter counts: A 3D sequence of length-2 tuples containing the counts and QPD bit information
            from each sub-experiment
        :parameter coefficients: A sequence of coefficients, such that each coefficient is associated
            with one unique sample. The length of ``coefficients`` should equal
            the length of ``counts``
        :parameter observables: The observable(s) for which the expectation values will be calculated.
